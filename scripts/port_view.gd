...
func _draw() -> void:
	...
	if w <= 8.0 or h <= 8.0:
		return
	var sun:float = 0.5 + 0.5 * sin((day_phase - 0.25) * TAU)
	var sky:Color = Color("#243b50").lerp(Color("#82a9bd"), sun)
	var sea:Color = Color("#17364b").lerp(Color("#2d6f87"), sun)
	var land:Color = Color("#4b493f").lerp(Color("#77705d"), sun)
	draw_rect(Rect2(0, 0, w, h * 0.34), sky)
	draw_rect(Rect2(0, h * 0.34, w, h * 0.22), land)
	draw_rect(Rect2(0, h * 0.56, w, h * 0.44), sea)
	for i in range(14):
		var bx:float = 10.0 + float(i) * (w / 14.0)
		var bh:float = 35.0 + float((i * 31) % 70)
		var bw:float = 42.0 + float((i * 17) % 30)
		var c:Color = Color("#3f4342").lerp(Color("#716c60"), sun)
	...
	var buildings:int = mini(11, 5 + port_level * 2)
	for i in range(buildings):
		var bx:float = 18.0 + float(i) * maxf(78.0, (w - 40.0) / float(buildings))
		var by:float = h * 0.37
		var bw:float = 62.0
		var bh:float = 52.0 + float((i * 13) % 25)
		...
		for wx in range(2):
			for wy in range(2):
				var lit:bool = (i + wx + wy + int(sim_time * 0.1)) % 4 == 0 and sun < 0.45
				draw_rect(Rect2(bx + 10 + wx * 26, by - bh + 12 + wy * 18, 8, 7), Color("#edca71") if lit else Color("#28363a"))
	for i in range(4):
		var px:float = 45.0 + float(i) * (w * 0.22)
		var py:float = h * 0.56
	...
	var train_x:float = fposmod(sim_time * (34.0 * activity_level), w + 260.0) - 220.0
	for i in range(5):
	...
	for i in range(4):
		var cx:float = fposmod(sim_time * (24.0 + i * 4.0) * activity_level + i * 210.0, w + 120.0) - 80.0
	...
	var ship_count:int = int(clampf(3.0 + activity_level * 2.5, 3.0, 8.0))
	for i in range(ship_count):
		var dir:float = 1.0 if i % 2 == 0 else -1.0
		var speed:float = 16.0 + i * 3.5
		var sx:float = fposmod(sim_time * speed * activity_level + i * 193.0, w + 250.0) - 130.0
		if dir < 0:
			sx = w - sx
		var sy:float = h * (0.62 + (i % 4) * 0.085)
		_draw_ship(Vector2(sx, sy), 0.62 + (i % 3) * 0.12, Color("#2a3035") if i % 2 == 0 else Color("#6b352e"), Color("#e6dac0"))
	for i in range(3):
		var tx:float = w - fposmod(sim_time * (37.0 + i * 7.0) * activity_level + i * 280.0, w + 100.0)
		var ty:float = h * (0.59 + i * 0.13)
		draw_rect(Rect2(tx, ty, 48, 12), Color("#9e4d35"))
		draw_rect(Rect2(tx + 15, ty - 13, 20, 13), Color("#e7dfc9"))
	for i in range(30):
		var yy:float = h * 0.585 + float(i % 13) * 20.0
		var xx:float = fposmod(float(i * 91) + sim_time * (10.0 + i % 4), w + 60.0) - 30.0
...
func _draw_smoke(origin:Vector2, seed:int) -> void:
	for j in range(4):
		var drift:float = fposmod(sim_time * (5.0 + seed * 0.1) + j * 13.0, 48.0)
		var p:Vector2 = origin + Vector2(drift * 0.35, -drift - j * 5.0)
		draw_circle(p, 5.0 + j * 1.5, Color(0.25, 0.27, 0.28, 0.24))

func _draw_crane(base:Vector2, seed:int) -> void:
	var swing:float = sin(sim_time * 0.75 + seed) * 11.0
	var gold:Color = Color("#d4a43c")
	...
	var hook_x:float = base.x + 36 + swing
...
func _draw_ship(pos:Vector2, scale_v:float, hull:Color, cabin:Color) -> void:
	var s:float = scale_v
	var pts:PackedVector2Array = PackedVector2Array([pos, pos + Vector2(118, 0) * s, pos + Vector2(100, 20) * s, pos + Vector2(18, 20) * s])
	...
	draw_line(pos + Vector2(80, -20) * s, pos + Vector2(80, -44) * s, Color("#242729"), maxf(1.0, 2.0 * s))
	draw_line(pos + Vector2(-8, 15) * s, pos + Vector2(-38, 15) * s, Color(0.7,0.9,0.95,0.35), maxf(1.0,2.0*s))
